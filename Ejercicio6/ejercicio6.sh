...
ayuda() {
	...
	echo "datos de los países/ciudades con el formato indicado arriba, y opcionalmente,"
	...
	echo
	echo "Ejemplos:"
	echo "$0 personas.db ubicaciones.db salida.out"
}
...
if [[ $# -lt 2 || $# -gt 3 ]]; then
	errorLlamada "El script necesita exactamente 2 ó 3 parámetros."
fi
...
# Definimos otro patron para AWK, que duplique el país al comienzo de cada línea.
patronAWK='{
				pais = $4;
				for(i = 5; i <= NF; i++) {
					pais = pais" "$i;
...
tmp=$(awk -F", " "$patronAWK" "$archivoTemporal")

# Tomamos dicha la salida y la ordenamos por país y nombre.
echo "$tmp" | sort -u -d -k1 -k2 > "$archivoTemporal"
...
awk -F", " "$patronAWK" "$archivoTemporal" > "$archivoSalida"
